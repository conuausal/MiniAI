...
LENGTH_GUIDES = {
    "short": ("短文：全文 800-1200 字，2-3 个章节", 3),
    "medium": ("中等：全文 1800-2800 字，4-5 个章节", 4),
    "long": ("长文：全文 3500-5000 字，6-7 个章节", 6),
}
...
7. 【硬性要求】用户指定的全文总字数必须严格遵守（允许 ±20% 偏差）：字数不足时扩展论述、补充案例与细节；超出时精简。宁可多写也不要低于下限。"""
...
请撰写完整文章。"""
    ...
    async for kind, text in stream_chat(
        ...
        messages=messages,
        temperature=0.7,
        max_tokens=8000,  # 5000 字中文 ≈ 6000+ token，上限太低会把长文截断
        user_keys=user_keys,
        custom_providers=custom_providers,
    ):
        if kind != "delta":
