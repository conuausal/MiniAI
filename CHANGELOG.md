...
- **真·中文语义检索启用**：`bge-small-zh-v1.5` 落地本地目录（`data/models/`）离线加载，
  彻底绕开 HF 网络问题；提供一键下载脚本 + 向量库重建脚本（自动备份/重灌/幂等）
- **检索过程可视化**：新 SSE 事件 `rag_hits`，前端青色卡片展示命中片段（来源/相关度/预览）
- **未命中明示**：知识库无命中时提示"本次回答基于模型通识"，消除"以为答案来自文档"的误解
- **知识库严格模式**（📕 严格开关）：仅依据检索片段回答，未覆盖直接说"没有相关内容"，禁止编造；
  引用片段带 [n] 序号
- **修复知识库删除功能**：chromadb 0.5.x 多条件 where 必须用 `$and` 包裹，
  原 `where={doc_id, user_id}` 写法导致删除接口自项目上线以来一直静默失败

### 🔧 其他
- 上传入库增加 20MB/格式校验的健壮性；重排/检索失败自动降级链路不变
